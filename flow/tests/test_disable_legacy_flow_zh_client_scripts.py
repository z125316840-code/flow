...
class TestDisableLegacyFlowZhClientScripts(TestCase):
	def test_disables_only_returned_legacy_scripts(self):
		...
		with patch.object(disable_legacy_flow_zh_client_scripts, "frappe") as frappe:
			frappe.get_all.return_value = enabled
			disable_legacy_flow_zh_client_scripts.execute()

		self.assertEqual(frappe.get_all.call_args.args, ("Client Script",))
		self.assertEqual(frappe.get_all.call_args.kwargs["filters"]["enabled"], 1)
		self.assertEqual(
			set(frappe.get_all.call_args.kwargs["filters"]["name"][1]),
